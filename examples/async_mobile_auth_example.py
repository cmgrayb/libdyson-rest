"""
Example usage of async mobile authentication with Dyson REST API.

This demonstrates the async mobile authentication flow available on the China (CN)
region server. Mobile authentication uses SMS OTP codes instead of email OTP codes.
...
async def main() -> None:
    """Demonstrate async mobile authentication flow."""
    ...
    logger.info("=== Dyson Async Mobile Authentication Example ===")
    ...
    try:
        ...
        user_status = await client.get_user_status_mobile(mobile)
        ...
        challenge = await client.begin_login_mobile(mobile)
        ...
        # Note: input() is blocking, so in a real async app
        # you'd want to use async input or a GUI.
        ...
        if devices:
            ...
            iot_data = await client.get_iot_data(device.serial)
        ...
        # Step 9: Close the async client properly
        logger.info("Step 9: Closing async client...")
        await client.close()
        logger.info("✓ Client closed")
        logger.info("")

        logger.info(
            "=== Async Mobile Authentication Example Completed Successfully ==="
        )
